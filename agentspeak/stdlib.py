...
def _broadcast(agent, term, intention):
    ...
    if ilf.functor == "tell":
        ...
        raise agentspeak.PysonError("unknown illocutionary force: %s" % ilf)
...
def _print(agent, term, intention, _color_map={}, _current_color=[0]):
    ...
    memo = {}
    text = " ".join(pyson_str(agentspeak.freeze(t, intention.scope, memo)) for t in term.args)
...
def _substring(agent, term, intention):
    needle = pyson_str(agentspeak.grounded(term.args[0], intention.scope))
    haystack = pyson_str(agentspeak.grounded(term.args[1], intention.scope))
...
def _wait(agent, term, intention):
    ...
    if not (millis is None or agentspeak.is_number(millis)):
        raise agentspeak.PysonError("expected timeout for .wait to be numeric")
    if not (event is None or agentspeak.is_string(event)):
        raise agentspeak.PysonError("expected event for .wait to be a string")
